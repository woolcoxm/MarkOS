...
scp -O $SSH_OPTS "$BIN" "$PI:/data/markos-engine.new" >/dev/null
ssh $SSH_OPTS "$PI" '
	...
	grep -q "exec /data/bin/markos-engine" /etc/service/markos-engine/run || sed -i "s|exec /usr/bin/markos-engine|exec /data/bin/markos-engine|" /etc/service/markos-engine/run
