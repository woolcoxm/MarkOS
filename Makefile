...
## Phase 11 acceptance: sustained real generation — 3 sequential full
## GEN runs (identical workload) with STATS accounting, wall-clock
## cross-check, token-exact outputs, and per-run latency drift bound.
## The engine path performs zero heap allocations (statics only), so the
## drift check plus a live connection IS the leak/latency-growth probe.
test-gensoak: export RUSTFLAGS = -C target-feature=+dotprod
test-gensoak: image-virt $(REAL_IMG)
	$(MAKE) image-virt KERNEL_FEATURES=selftest-net
	python3 scripts/forward_ref.py $(MODEL_FILE) $(HOME)/.markos-tests/gen-expected.txt --gen 2
	bash scripts/kill_qemu.sh; sleep 1; \
